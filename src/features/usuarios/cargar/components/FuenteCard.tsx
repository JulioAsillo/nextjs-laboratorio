...
export function FuenteCard({
  ...
  const recompute = useCallback(() => {
    setAnyUploaded(fuente.slots.some((s) => isSlotUploaded(fuente.id, s.fileName)));
  }, [fuente]);
  ...
  async function handleDelete() {
    ...
    setDelError(null);
    try {
      await deleteFuente(fuente.appsKey);
      clearFuente(fuente.id);
      setConfirmDel(false);
      setLocalTick((t) => t + 1); // remonta slots + recomputa indicador
      onDeleted?.();
    } catch (e) {
  ...
  return (
    ...
    >
      ...
      {/* Strip: archivos en backend + eliminar */}
          ...
          {!confirmDel ? (
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                ...
                title="Eliminar el Excel subido al backend"
...
interface MergeState {
  phase: MergePhase; sourceCount: number; totalRows: number; message?: string; error?: string;
}
...
function SlotUploader({ slot, fuenteId, showLabel, onUploaded }: SlotUploaderProps) {
  ...
  useEffect(() => {
    setPrevUploaded(isSlotUploaded(fuenteId, slot.fileName));
  ...
  // Mantiene una referencia viva a handleUpload para el botón global "Subir todos".
  const runRef = useRef<() => Promise<void>>(async () => {});
  useEffect(() => { runRef.current = handleUpload; });

  // Registra este slot en el lote global mientras tenga archivos válidos pendientes.
  useEffect(() => {
  ...
  async function handleUpload() {
    ...
    if (slot.multiple) {
      ...
      try {
        const merged = await mergeFilesToXlsx(validFiles.map((v) => v.file), slot.columns, `${slot.fileName}.xlsx`);
        ...
        markUploaded(fuenteId, slot.fileName);
