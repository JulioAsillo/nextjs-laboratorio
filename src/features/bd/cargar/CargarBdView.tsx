...
import { FuentesCargadasPanel } from '@/features/usuarios/cargar/components/FuentesCargadasPanel';
...
export default function CargarBdView() {
  ...
  const labelByKey = useMemo(() => {
    ...
    for (const f of bdFuentes) if (f.appsKey) m.set(f.appsKey, f.label);
    return m;
  }, []);

  const kindByKey = useMemo(() => {
    const m = new Map<string, 'dbs' | 'shared'>();
    for (const f of bdFuentes) if (f.appsKey) m.set(f.appsKey, f.kind);
    return m;
  }, []);

  const loadOne = useCallback(async (appsKey: string) => {
    const kind = kindByKey.get(appsKey) ?? 'shared';
    setLoadingKey(appsKey);
    setStatus((prev) => ({ ...prev, [appsKey]: 'loading' }));
    try {
      const res = (await mutate(bdDatosKey(appsKey), fetchDatosBd(kind, appsKey), { revalidate: false })) as DatosResult | undefined;
  ...
  return (
    <AppShell
      ...
      breadcrumb={['Certificación de Base de Datos', 'Cargar Información']}
      actions={
        <>
    ...
    >
      ...
      <div className="flex gap-6">
        <div className="flex min-w-0 flex-1 flex-col gap-8">
          {BD_GROUPS.map((group) => {
            const items = bdFuentes.filter((f) => f.group === group.key);
            return (
        ...
        <FuentesCargadasPanel cargadas={cargadas} errores={errores} total={appsKeys.length} onView={(appsKey, label) => setModal({ kind: kindByKey.get(appsKey) ?? 'shared', appsKey, title: label })} />
      </div>

      {modal && <DatosModalBd kind={modal.kind} appsKey={modal.appsKey} title={modal.title} onClose={() => setModal(null)} />}
    </AppShell>
  );
}
